...
\* Label finished of process trustnet_out at line 262 col 19 changed to finished_
...
SAFETYCHECK == 
    ...
    \*/\ \A m \in chan["messagecheck"] : Len(m.text) <= MAXMODBUSSIZE \*only messages with a valid length make it to the checking module
    /\ \A m \in chan["finished_trustnet"] : IsModbus(NumTupleToStrTuple(m)) \*only properly formed modbus is sent to trustnet
    /\ \A m \in chan["finished_untrustnet"] : GetHMAC(m) = HMAC(m,m) \*only properly signed messages are sent to untrustnet
    /\ \A m \in chan["finished_untrustnet"] : IsModbus(SubSeq(m,66,Len(m))) \*only properly formed modbus is sent with signature to untrustnet
=============================================================================
\* Modification History
\* Last modified Wed Oct 24 21:15:18 EDT 2018 by mssabr01
