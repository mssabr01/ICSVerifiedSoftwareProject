...
SAFETYCHECK ==
    /\ IsSSW(NumTupleToStrTuple(applicationBuffer)) \/ applicationBuffer = <<>> \*only valid modbus makes it to the app buffer
    /\ rx = TRUE <=> IsSSW(NumTupleToStrTuple(applicationBuffer)) \*flag is raised if and only if there is valid modbus in app buffer

LIVELINESS ==
    /\ IsSSW(NumTupleToStrTuple(incMessage)) ~> IsSSW(NumTupleToStrTuple(applicationBuffer)) \* if the message is modbus then it gets to the app buffer
    /\ IsSSW(NumTupleToStrTuple(incMessage)) ~> rx = TRUE \* if valid modbus comes through then it gets flagged for the application to consume
